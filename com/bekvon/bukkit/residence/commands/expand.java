...
public class expand
implements cmd {
    @CommandAnnotation(simple=1, priority=2000)
    @Override
    public boolean perform(String[] args, boolean resadmin2, Command command2, CommandSender sender) {
        ...
        if (res.isSubzone() && !player.hasPermission("residence.expand.subzone") && !resadmin2) {
            Residence.msg((CommandSender)player, lm.Subzone_CantExpand, new Object[0]);
            return false;
        }
        if (!(res.isSubzone() || player.hasPermission("residence.expand") || resadmin2)) {
            Residence.msg((CommandSender)player, lm.Residence_CantExpandResidence, new Object[0]);
            return false;
        }
        ...
        if (Residence.getSelectionManager().hasPlacedBoth(player.getName())) {
            if (Residence.wep != null && Residence.wepid == Residence.getConfigManager().getSelectionTooldID()) {
    ...
    public void getLocale(ConfigReader c, String path) {
        c.get(String.valueOf(path) + "Description", "Expands residence in direction you looking");
        c.get(String.valueOf(path) + "Info", Arrays.asList("&eUsage: &6/res expand (residence) [amount]", "Expands residence in direction you looking.", "Residence name is optional"));
